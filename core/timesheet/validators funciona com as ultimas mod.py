...
class TimesheetValidator:
    ...
    MAX_HOURS_PER_DAY = Decimal('16.00')   # Máximo por dia por colaborador
    MAX_RETROACTIVE_DAYS = 30              # Máximo de dias para registrar tarefas passadas
    MIN_HOURS_PER_DAY = Decimal('0.00')    # Mínimo por tarefa (0.5h = 30min)
    ...
    def validate_timesheet_data(
        ...
        """
        Valida todos os aspectos de uma timesheet com as novas regras
        Retorna: (is_valid, daily_totals, warnings)
        """
        ...
        try:
            ...
            for task_data in data.get('tasks', []):
                ...
                # Validação temporal (NOVO)
                if timesheet_date:
                    TimesheetValidator._validate_temporal_consistency(
                        timesheet_date, task_date, emp_id
                    )

                # Validação retroativa (NOVO)
                TimesheetValidator._validate_retroactive_date(task_date)

                # Validação de duplicidade ENTRE timesheets (NOVO)
                if emp_id:
                    TimesheetValidator._validate_cross_timesheet_duplicate(
                        emp_id, task_date, task_data, timesheet_id
                    )
            ...
            # 3. VALIDAÇÃO DE TOTAL POR DIA (NOVA - AGREGADA)
            if validation_level == "strict" and emp_id:
                for task_date, new_hours in daily_totals.items():
                    # Calcular total agregado do colaborador na data
                    total_daily = TimesheetValidator._get_aggregated_daily_total(
                        emp_id, task_date, timesheet_id, new_hours
                    )

                    # Validar limite máximo diário
                    if total_daily > TimesheetValidator.MAX_HOURS_PER_DAY:
                        raise field_error(
                            "tasks",
                            f"Total diário em {task_date} seria {float(total_daily)}h "
                            f"(máximo permitido: {TimesheetValidator.MAX_HOURS_PER_DAY}h)"
                        )

                    # AVISOS OPCIONAIS (apenas se não for force_confirm)
                    if not force_confirm:
                        if total_daily > Decimal('8.00'):
                            warning_msg = (
                                f"⚠️ Total de horas no dia {task_date} é {float(total_daily)}h "
                                f"(superior às 8h normais). Deseja confirmar?"
                            )
                            daily_warnings[task_date].append(warning_msg)
                        elif total_daily < Decimal('8.00'):
                            warning_msg = (
                                f"⚠️ Total de horas no dia {task_date} é {float(total_daily)}h "
            ...
            # 4. Validação de duplicidade de timesheet na mesma data
    ...
    def _get_aggregated_daily_total(
        ...
        """
        Calcula o total agregado de horas de um colaborador em uma data
        Incluindo horas existentes + novas horas
        """
    ...
    def _validate_cross_timesheet_duplicate(
        ...
        """
        Valida se já existe tarefa idêntica em OUTRA timesheet do mesmo colaborador
        """
        duplicate_query = Task.objects.filter(
            timesheet__employee_id=employee_id,
            created_at=task_date,
        ...
        if exclude_timesheet_id:
            duplicate_query = duplicate_query.exclude(timesheet_id=exclude_timesheet_id)
        ...
        if duplicate_query.exists():
            ...
            raise field_error(
                "tasks",
                f"Tarefa já registrada: dia {task_date}, projeto '{project.name}', "
                f"atividade '{activity.name}' na timesheet #{duplicate.timesheet_id}"
            )
    ...
    def _validate_temporal_consistency(
        ...
        # 4. Timesheet não pode ser anterior à primeira tarefa
        # (já coberto pela regra 3)
    ...
    def _validate_task(task_data: dict, validation_level: str = "strict"):
        ...
        task_hour1 = Decimal(str(task_data['hour']))  # '0.25' → Decimal('0.25')

        print(task_hour, task_hour1, 'diferença')

        # Horas devem ser positivas
        if task_hour <= Decimal('0.00'):
            raise field_error("hour", "Horas da tarefa devem ser maiores que zero 1")
    ...
    def validate_user_permission(timesheet: Timesheet, user: User):
        print(f"👤 Usuário atual: {user.id} - {user.username}")
        ...
        print(f"👤 Dono do timesheet: {timesheet.employee_id}")