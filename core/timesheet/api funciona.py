...
@router.get("/timesheets/my", response=PaginatedTimesheetResponse)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@router.get("/timesheets/department", response=PaginatedTimesheetResponse)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@router.get("/timesheets/all", response=PaginatedTimesheetResponse)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@router.get("/timesheets/{id}", response=TimesheetOut)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@router.put("/timesheets/{id}", response={200: TimesheetOut, 400: dict, 207: dict})
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
@router.post("/timesheets", response={200: TimesheetOut, 400: dict, 207: dict})
    ...
    try:
        ...
        if validation_level == "strict":
            ...
            if warnings and not data.force_confirm:
                # Gera mensagem adaptável (mesma lógica do update)
                if any("superior às 8h normais" in w for w in warnings):
                    message = "⚠️ Total diário superior às 8h normais. Confirme para prosseguir."
                elif any("excede 8 horas" in w for w in warnings):
                    message = "⚠️ Algumas tarefas excedem 8 horas individuais. Confirme para prosseguir."
